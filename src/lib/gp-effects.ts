...
function animate(){
  ...
  boxGlow.addColorStop(0,'rgba(26,154,250,0.025)');boxGlow.addColorStop(1,'transparent');
  ...
  // Crosshairs
  const chR=maxR*0.14;
  X.globalAlpha=0.035;X.strokeStyle='#1a9afa';X.lineWidth=0.5;
  ...
  coreG.addColorStop(0,`rgba(26,154,250,${0.06*cp})`);coreG.addColorStop(1,'transparent');
  ...
  cg.addColorStop(0,'#c8e7ff');cg.addColorStop(1,'#1a9afa');
  ...
  X.strokeStyle=`rgba(26,154,250,${0.05*cp})`;X.lineWidth=0.5;X.stroke();
  ...
  if(!journey){journeyCooldown--;if(journeyCooldown<=0)startJourney(L);
    ...
    if(j.phase==='toData'){
      ...
      j.p+=j.speed*1.3;const ep=ease(j.p);
      drawP(j.engR+(j.decX-6-j.engR)*ep,cy,3,'#1a9afa',13,0.9);
      drawP(j.engR+(j.decX-6-j.engR)*ep,cy,1.2,'#c8e7ff',4,0.5);
      if(j.p>=1){j.phase='inDecision';j.p=0;}
    }
    else if(j.phase==='inDecision'){
      j.p+=0.05;const a=Math.sin(j.p*Math.PI);
      const dg=X.createRadialGradient(j.decX,cy,0,j.decX,cy,18);
      dg.addColorStop(0,`rgba(26,154,250,${a*0.12})`);dg.addColorStop(1,'transparent');
      X.fillStyle=dg;X.beginPath();X.arc(j.decX,cy,18,0,Math.PI*2);X.fill();
      drawP(j.decX,cy,3,'#1a9afa',10,0.8);
      ...
      drawP(x,y,3.5,'#5bb8ff',15,0.9);drawP(x,y,1.5,'#ffffff',4,0.45);
      X.beginPath();X.moveTo(j.decX,cy);
      X.quadraticCurveTo((j.decX+j.out.x)/2,j.out.y,j.out.x-12,j.out.y);
      X.strokeStyle=`rgba(26,154,250,${0.05*(1-ep*0.5)})`;X.lineWidth=1;X.stroke();
      if(j.p>=1){
        ...
        for(let i=0;i<8;i++){const angle=(Math.PI*2/8)*i+Math.random()*0.2;
          j.sparkleList.push({x:j.out.x,y:j.out.y,
            ...
            life:1,size:0.5+Math.random()*0.6,color:Math.random()>0.5?'#5bb8ff':'#67e8f9'});}
      }
    }
